...
struct cwk
{
   ...
   */
  enum cwk_segment_type get_segment_type(
   ...
   */
  enum cwk_path_style guess_style(const char *path);
   ...
   */
  void set_style(enum cwk_path_style style);
   ...
   */
  enum cwk_path_style get_style(void);
 ...
 */
  ...
  enum cwk_path_style path_style = CWK_STYLE_WINDOWS;
#else
...
inline size_t output_sized(char *buffer, size_t buffer_size,
...
inline size_t output_current(char *buffer, size_t buffer_size,
  size_t position)
{
...
inline size_t output_back(char *buffer, size_t buffer_size,
  size_t position)
{
...
inline size_t output_separator(char *buffer, size_t buffer_size,
  size_t position)
{
  // We output a separator, which is a single character.
  return output_sized(buffer, buffer_size, position,
    separators[path_style], 1);
}

inline size_t output_dot(char *buffer, size_t buffer_size,
  size_t position)
{
...
inline void terminate_output(char *buffer, size_t buffer_size,
  size_t pos)
{
  if (buffer_size > 0) {
    if (pos >= buffer_size) {
...
inline bool is_string_equal(const char *first, const char *second,
  size_t n)
{
...
inline bool get_last_segment_without_root(const char *path,
  struct cwk_segment *segment)
{
...
inline bool get_first_segment_joined(const char **paths,
  struct cwk_segment_joined *sj)
{
  ...
  while (paths[sj->path_index] != NULL &&
         (result = get_first_segment(paths[sj->path_index],
            &sj->segment)) == false) {
    ++sj->path_index;
  }
...
inline bool get_next_segment_joined(struct cwk_segment_joined *sj)
{
  ...
  do {
    ...
    result = get_first_segment_without_root(sj->paths[sj->path_index],
...
inline bool get_previous_segment_joined(struct cwk_segment_joined *sj)
{
  ...
  do {
    ...
    if (sj->path_index == 0) {
      result = get_last_segment(sj->paths[sj->path_index],
        &sj->segment);
    } else {
      result = get_last_segment_without_root(sj->paths[sj->path_index],
        &sj->segment);
    }
...
inline bool
segment_will_be_removed(const struct cwk_segment_joined *sj,
  bool absolute)
{
  enum cwk_segment_type type;
...
inline bool
segment_joined_skip_invisible(struct cwk_segment_joined *sj,
  bool absolute)
{
...
inline size_t join_and_normalize_multiple(const char **paths,
  char *buffer, size_t buffer_size)
{
  size_t pos;
  ...
  do {
    ...
    // here.
    pos += output_sized(buffer, buffer_size, pos, sj.segment.begin,
      sj.segment.size);
  } while (get_next_segment_joined(&sj));
  ...
  bool *other_available)
{
  ...
  do {
    ...
    *other_available = segment_joined_skip_invisible(osj, absolute);
    ...
    // since they diverge.
    if (!is_string_equal(bsj->segment.begin, osj->segment.begin,
          bsj->segment.size)) {
      break;
    }
