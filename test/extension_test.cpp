...
int extension_change_with_trailing_slash()
{
  ...
  n = cwk_path.change_extension(buffer, ".md", buffer, sizeof(buffer));
  if (n != 16) {
    return EXIT_FAILURE;
  }
...
int extension_change_overlap_long()
{
  ...
  n = cwk_path.change_extension(buffer, ".text", buffer, sizeof(buffer));
  if (n != 11) {
    return EXIT_FAILURE;
  }
...
int extension_change_with_dot()
{
  ...
  n = cwk_path.change_extension("/test.txt", ".md", buffer, sizeof(buffer));
  if (n != 8) {
    return EXIT_FAILURE;
  }
...
int extension_change_remove()
{
  ...
  n = cwk_path.change_extension("/test.txt", "", buffer, sizeof(buffer));
  if (n != 5) {
    return EXIT_FAILURE;
  }
...
{
  cwk_path.set_style(CWK_STYLE_UNIX);
  if (cwk_path.has_extension("/my/path")) {
    return EXIT_FAILURE;
  }
...
{
  cwk_path.set_style(CWK_STYLE_UNIX);
  if (!cwk_path.has_extension("/my/path.txt")) {
    return EXIT_FAILURE;
  }
...
int extension_get_multiple()
{
  ...
  if (!cwk_path.get_extension(path, &extension, &length)) {
    return EXIT_FAILURE;
  }
