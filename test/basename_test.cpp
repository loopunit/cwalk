...
int basename_change_trim_only_root()
{
  ...
  n = cwk_path.change_basename("/", "///another.txt///", buffer,
    sizeof(buffer));
  if (n != 12) {
    return EXIT_FAILURE;
  }
...
int basename_change_trim()
{
  ...
  n = cwk_path.change_basename("/test.txt", "///another.txt///", buffer,
    sizeof(buffer));
  if (n != 12) {
    return EXIT_FAILURE;
  }
...
int basename_change_empty_basename()
{
  ...
  n = cwk_path.change_basename("C:\\test.txt", "", buffer, sizeof(buffer));
  if (n != 3) {
    return EXIT_FAILURE;
  }
...
int basename_change_only_root()
{
  ...
  n = cwk_path.change_basename("C:\\", "another.txt", buffer, sizeof(buffer));
  if (n != 14) {
    return EXIT_FAILURE;
  }
...
int basename_change_empty_path()
{
  ...
  n = cwk_path.change_basename("", "another.txt", buffer, sizeof(buffer));
  if (n != 11) {
    return EXIT_FAILURE;
  }
...
int basename_change_simple()
{
  ...
  n = cwk_path.change_basename("C:\\test.txt", "another.txt", buffer,
    sizeof(buffer));
  if (n != 14) {
    return EXIT_FAILURE;
  }
...
int basename_windows()
{
  ...
  cwk_path.set_style(CWK_STYLE_WINDOWS);
  path = "C:\\path\\test.txt";
  cwk_path.get_basename(path, &basename, &length);
