...
int windows_intersection_case()
{
  ...
  if (cwk_path.get_intersection("C:\\MYFOLDER\\MYILE.TXT",
...
int windows_get_root_relative()
{
  size_t size;
  cwk_path.set_style(CWK_STYLE_WINDOWS);
  cwk_path.get_root("C:file.txt", &size);
...
int windows_get_root_separator()
{
  size_t size;
  cwk_path.set_style(CWK_STYLE_WINDOWS);
  cwk_path.get_root("C:/this/is/a/test", &size);
...
int windows_get_unc_root()
{
  ...
  cwk_path.set_style(CWK_STYLE_WINDOWS);
  cwk_path.get_root("\\\\server\\share\\test.txt", &size);
...
int windows_get_root()
{
  ...
  cwk_path.set_style(CWK_STYLE_WINDOWS);
  cwk_path.get_root("C:\\test.txt", &size);
...
int windows_change_style()
{
  ...
  style = cwk_path.get_style();
  if (style == CWK_STYLE_WINDOWS) {
    cwk_path.set_style(CWK_STYLE_UNIX);
    if (cwk_path.get_style() != CWK_STYLE_UNIX) {
      return EXIT_FAILURE;
    }
  } else {
    cwk_path.set_style(CWK_STYLE_WINDOWS);
    if (cwk_path.get_style() != CWK_STYLE_WINDOWS) {
      return EXIT_FAILURE;
    }
  }
