...
int segment_change_overlap()
{
  ...
  if (!cwk_path.get_first_segment(buffer, &segment)) {
    return EXIT_FAILURE;
  }

  if (!cwk_path.get_next_segment(&segment)) {
    return EXIT_FAILURE;
  }

  cwk_path.change_segment(&segment, "longer_segment", buffer, sizeof(buffer));
...
int segment_change_with_separator()
{
  ...
  if (!cwk_path.get_first_segment(path, &segment)) {
    return EXIT_FAILURE;
  }
  ...
  cwk_path.change_segment(&segment, "other\\fancy", buffer, sizeof(buffer));
...
int segment_change_empty()
{
  ...
  cwk_path.change_segment(&segment, "", buffer, sizeof(buffer));
...
int segment_change_trim()
{
  ...
  cwk_path.change_segment(&segment, "//other/\\\\", buffer, sizeof(buffer));
...
int segment_change_last()
{
  ...
  path = "C:\\this\\cool\\path\\";
  if (!cwk_path.get_last_segment(path, &segment)) {
    return EXIT_FAILURE;
  }
...
int segment_back_with_root()
{
  ...
  if (!cwk_path.get_previous_segment(&segment)) {
    return EXIT_FAILURE;
  }
...
int segment_type()
{
  ...
  if (cwk_path.get_segment_type(&segment) != CWK_NORMAL) {
    return EXIT_FAILURE;
  }
  ...
  if (cwk_path.get_segment_type(&segment) != CWK_CURRENT) {
    return EXIT_FAILURE;
  }
  ...
  if (cwk_path.get_segment_type(&segment) != CWK_BACK) {
    return EXIT_FAILURE;
  }
